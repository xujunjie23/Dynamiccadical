...
static double avg_glue = 0;//衡量子句质量，考虑是否mab
int delta_clauses_added = stats.current.redundant - last_clauses_added;//当前累积的冲突子句数-上一次记录时冗余子句数，也就是新增学习子句数
int delta_clauses_deleted = stats.reductions - last_clauses_deleted;//自上次记录后，删除的子句数
int change_score = delta_clauses_added + delta_clauses_deleted * 100;//计算最近子句变更分数，因为删除子句影响通常比添加子句大得多。如果太大就说明公式变化很大了
...
if (num_decisions_D < DSAT_SAMPLE_CNT && num_decisions_D % 100 == 0) {//决策上限100000，每100次采样一次，也就是决策100次换一下配置
  ...
  if (learned)
    ...
    avg_glue = 0;
  printf("avg_glue: %f\n", avg_glue);
  // Action execution随机选择一个动作，并根据动作调整配置
  int cur_action = rand() % tot_actions;//一个整数，tot_actions=bool_num*2*int_num*3
  printf("cur_action: %d\n", cur_action);
  int* action_list = dsat_get_actions(cur_action);//根据动作索引获取配置调整方案，返回int*，表示每个可调参数调整方式
  ...
  for (int i = 0; i < DSAT_NO_CONFIGS; i++) {
    if (DSAT_CONFIG_TYPE[i] == BOOL_CONFIG) {//对于bool，采样会决定是将其值设为 0 还是 1
      ...
      cadical_set_option(&opts, DSAT_CONFIG[i], config_value);//真正修改
      cur_config_values[i] = config_value;//存储当前
    }
  }
...
// Step 2. Decision 首先在1000次采样，也就是100000决策后才启用mab，启用mab后如果公式变化太大就要重置一下轮次，轮次要足够（100000）才停止mab，不同于上面，现在决策一次改一次配置
  ...
  if (change_score > DSAT_CHANGE_THRESHOLD) {//若change_score过大，说明最近配置对sat公式影响较大
    ...
    mab_in_process = 0;//重新开始mab采样，放弃之前新增子句数和之前删除子句数，因为之前变化子句数可能太旧了
  } else {
    mab_in_process += 1;
    mab_in_process %= (int)1e7+1;
  }

  if (mab_in_process % DSAT_DECISION_CNT == 0) {//计算reward却是决策100次才计算一次，因为这是反映最近100次的平均粘合度
    if (learned)
      avg_glue = (double)tot_glue / learned;//和上面一样计算
    else
      avg_glue = 0;
  }
  ...
  // Update UCB UCB 值考虑了该选项的奖励与选择次数的比例，同时也考虑了探索的需要
  for (int i = 0; i < tot_actions; i++) {
    if (mab_selected_D[i] == 0) {
      ucb_D[i] = 1e9;  // 优先探索未选过的action
    } else {
...
// MAB Reset 变更分数达到一定阈值，或者决策次数超过一定数量，则重置 MAB 状态
num_decisions_D += 1;
//又计算一次的是tot
...
if (tot_change_score > mab_reset_threshold) {
  mab_reset_threshold += (stats.current.irredundant + stats.current.redundant) * 10;//若变更分数过大，发生重置，则增大重置阈值，防止频繁重置
  if (num_decisions_D > DSAT_SAMPLE_CNT)
    num_decisions_D = 0;//重置时，若决策次数超了阈值，将次数清0
}
  return res;
}
